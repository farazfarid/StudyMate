...
const ToDo = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center">
      ...
      <div className="grid grid-cols-3 m-20 w-4/5 h-4/5 gap-4">
  ...
  <Droppable droppableId={id}>
    {(provided) => (
      <div className="p-6 flex flex-col mt-4">
        ...
        <div
          className="bg-black/20 rounded-lg p-4 flex flex-col flex-grow mt-4"
...
const Item = ({ text, index, onDelete }) => (
  <Draggable draggableId={text} index={index}>
    {(provided) => (
      <div
        className="bg-black/20 rounded p-2 mr-4 transition duration-300 ease-out mt-4 hover:bg-white/20 hover:transition hover:duration-100 hover:ease-in flex items-center justify-between"
        ref={provided.innerRef}
...
const AddTodoInput = ({ value, onChange, onAdd }) => (
  <div className="p-6 flex flex-col mt-4">
    <h2 className="m-0 p-4">Add Todo Item</h2>
    <input
      type="text"
      value={value}
      onChange={onChange}
    ...
    />
    <button className="bg-green-500 text-white p-2 mt-2" onClick={onAdd}>
      Add Todo
    </button>
  </div>
);
